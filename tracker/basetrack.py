...
from tracker.kalman_filter import KalmanFilter, NaiveKalmanFilter, BoTSORTKalmanFilter, NSAKalmanFilter
...
class BaseTracker(object):
    ...
    def update(self, det_results, ori_img):
        ...
        matched_pair, u_track, u_detection = matching.linear_assignment(IoU_mat, thresh=0.8)
        ...
        matched_pair1, u_track1, u_detection1 = matching.linear_assignment(IoU_mat, thresh=0.5)
        ...
        for idet in u_detection1:  # for unmatched detection
            newtrack = u_det[idet]
            if newtrack.score > self.det_thresh + 0.12:  # conf is enough high
                newtrack.activate(self.frame_id)
                activated_stracks.append(newtrack)
