import numpy as np
from tracker.basetrack import TrackState, STrack, BaseTracker
from tracker.kalman_filter import KalmanFilter, NaiveKalmanFilter, chi2inv95
from tracker.reid_models.deepsort_reid import Extractor
...
class DeepSORT(BaseTracker):
    ...
    def gate_cost_matrix(self, cost_matrix, tracks, dets, max_apperance_thresh=0.15, gated_cost=1e5,
                         only_position=False):
        """
    ...
    def update(self, det_results, ori_img):
        ...
        if det_results.shape[0] > 0:
            ...
            detections = [
                STrack(cls, STrack.xywh2tlwh(xywh), score, kalman_format=self.opts.kalman_format, feature=feature,
                       use_avg_of_feature=False)
                for (cls, xywh, score, feature) in
                zip(det_results[:, -1], det_results[:, :4], det_results[:, 4], features)]
        ...
        matched_pair0, u_tracks0_idx, u_dets0_idx = matching.matching_cascade(self.gated_metric, 0.7,
                                                                              self.max_time_lost,
        ...
        # match
        matched_pair1, u_tracks1_idx, u_dets1_idx = matching.linear_assignment(IoU_dist, thresh=0.65)
        ...
        matched_pair2, u_tracks2_idx, u_det2_idx = matching.linear_assignment(IoU_dist, thresh=0.4)
        ...
        for idx in u_det2_idx:
            det = u_det1[idx]
            if det.score > self.det_thresh + 0.12:
